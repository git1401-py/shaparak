      ...
      <div class="accordion" id="accordionPanelsStayOpenExample">
        <div class="accordion-item" v-for="item in items" :key="item.id">
          ...
          <h2 class="accordion-header" :id="item.id">
            <button
              class="accordion-button text-end"
              type="button"
          ...
          >
            <div class="accordion-body">
              <div class="row">
                <div class="col-12">
                  <span style="font-weight: 200">{{ item.answer }}</span>
                  <br />
                </div>
              </div>
            </div>
...
export default {
  setup() {
    ...
    async function fetchItems() {
      // loading.value = true;
      await store.dispatch("questionStore/fetchItems");
      // loading.value = false;
    }
    const items = computed(() => store.getters["questionStore/allItems"]);
    return {
      items,
    };
  },
